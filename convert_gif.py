...
def check_time(time):
    ...
    if not isinstance(time[0], float) and time[0] is not None:
        quit('Ошибка: значение времени должно быть целочисленным или дробным')
    if not isinstance(time[1], float) and time[1] is not None:
        quit('Ошибка: значение времени должно быть целочисленным или дробным')
...
def get_args():
    ...
    parser.add_argument('--time', '-t', nargs='*', default=None,
...
def convert_time(new_time_gif):
    if new_time_gif is None:
        return None

    if new_time_gif[0] == 'None':
        new_time_gif[0] = None
    else:
        new_time_gif[0] = float(new_time_gif[0])
    if new_time_gif[1] == 'None':
        new_time_gif[1] = None
    else:
        new_time_gif[1] = float(new_time_gif[1])
    return new_time_gif
...
def main():
    ...
    python convert_gif.py --input video.mp4 --output result.gif --time 2 None
    ...
    new_time_gif = convert_time(new_time_gif)
